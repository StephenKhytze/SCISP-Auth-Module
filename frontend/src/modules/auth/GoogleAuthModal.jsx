...
export default function GoogleAuthModal({ 
  ...
  const handleSubmit = async (e) => {
    ...
    if (!cleanEmail) {
      setStatusInfo({
        type: 'error',
        message: 'Please enter your Google email address.'
      });
      return;
    }
    ...
    try {
      ...
      if (errorCode === 'REGISTRATION_PENDING') {
        ...
        setStatusInfo({
          type: 'not_found',
          message: data.message || `No registered ABC School account found for ${cleanEmail}.`,
          email: cleanEmail
        });
        ...
        setStatusInfo({
          type: 'error',
          message: data.message || 'Unable to sign in with this Google account. Please verify credentials.'
        });
      }
    } finally {
      setLoading(false);
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 font-medium mt-0.5">
                Single Sign-On for ABC School Portal
              </p>
            </div>
          </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. kirsten.estiva@abc.edu.ph or @gmail.com"
                  className="w-full px-4 py-3 bg-slate-50 border border-slate-300 rounded-xl text-xs font-medium text-slate-900 focus:outline-none focus:ring-2 focus:ring-[#182848] transition-all"
                  required
                  autoFocus
                />
                <p className="text-[11px] text-slate-500 mt-1.5">
                  Sign in with the verified Google email linked to your school account.
                </p>
              </div>
              ...
              <div className="flex items-center space-x-2 pt-2">
                <button
                  type="button"
                  onClick={handleClose}
                  className="flex-1 py-3 border border-slate-300 hover:bg-slate-50 text-slate-700 rounded-xl font-bold text-xs transition-colors cursor-pointer"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  disabled={loading}
                  className="flex-1 py-3 bg-[#182848] hover:bg-[#111d35] text-white rounded-xl font-bold text-xs transition-all shadow-md hover:shadow-lg disabled:opacity-50 flex items-center justify-center space-x-1.5 cursor-pointer"
                >
